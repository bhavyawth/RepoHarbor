...
export default function Navbar() {
  ...
  if (!user) {
    return (
      <nav className="pointer-events-none fixed inset-x-0 top-5 z-40 px-3 sm:px-8 lg:px-12">
        <div className="pointer-events-auto mx-auto flex w-full max-w-5xl items-center justify-between rounded-full border border-slate-300/80 bg-white/75 px-4 sm:px-6 py-2.5 shadow-sm backdrop-blur-md dark:border-indigo-900/30 dark:bg-slate-950/80">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <Button
              ...
              className="hidden sm:inline-flex transform-gpu cursor-pointer transition-transform duration-150 hover:scale-[1.02] active:scale-[0.98] rounded-full px-5 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100/80 dark:text-slate-200 dark:hover:bg-slate-800/80"
            >
              Features
            </Button>
            <Button
              onClick={handleGithubLogin}
              className="rounded-full cursor-pointer bg-slate-900 px-4 sm:px-5 py-2 text-xs sm:text-sm font-medium text-white hover:bg-slate-800 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100 transform-gpu transition-transform duration-150 hover:scale-[1.02] active:scale-[0.98] whitespace-nowrap"
            >
              <span className="sm:hidden">Sign in</span>
              <span className="hidden sm:inline">Get Started</span>
            </Button>
            <AnimatedThemeToggler className="inline-flex shrink-0 cursor-pointer items-center justify-center transform-gpu transition-transform duration-150 hover:scale-[1.02] active:scale-[0.98]" />
          </div>
        </div>
      </nav>
    );
